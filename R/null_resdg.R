...
null_resdg <- function(
  ...
  focal_prop = .09,
  impact = -.417,
  item_params_a = c(1.188,.528,1.065,1.294,0.721,1.129,.339,1.685,.861,1.08,
  ...
  cat('Beginning RESDG Null Distribution Estimation.\n')
  ...
  for (i in 1:iterations){
    ...
    nonfocss <- (focal_sample/focal_prop)-focal_sample #see how formula was obtained in scanned notes in pub folder
    nonfocmean <- ((impact+1)-(impact*focal_prop))/(1-focal_prop) #see how formula was obtained in scanned notes in pub folder
    ...
    twopl <- function(iparam, theta, i){
      ((exp(iparam[i,2]*(theta-iparam[i,3])))/
         (1+(exp(iparam[i,2]*(theta-iparam[i,3])))))
    }
    ...
    data <- cbind(data,c(rep(1,focal_sample),rep(0,(totalss-focal_sample))))

    ttscore <- rowSums(data[,1:item_count])

    stratum_group <- cut(ttscore,breaks=seq(min(ttscore),max(ttscore),length.out=5),
    ...
    for(items_again in 1:item_count){
      ...
      for (s in unique(stratum_group)){
        ...
        sub_data_foc <- subset(sub_data,sub_data[,(item_count+1)]==1)

        if(length(sub_data_foc[,items_again]<10)){  # ignoring strata with less than 10 focal examinees
          brackets <- brackets
        }

        if(length(sub_data_foc[,items_again]>=10)){  # using only strata with 10 or more focal examinees
    ...
    null_resdg_est <- rbind(null_resdg_est,resdg_each_item)
